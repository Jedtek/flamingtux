...
void Config::loadConfigOptions() {
	...
	if (!config) {
		...
		tmp = "EES";
		config_options_->setEnableEventSounds(getOption(tmp));
		tmp = "ESR";
		config_options_->setESReceivedMessages(getOption(tmp));
		tmp = "ESB";
		config_options_->setESBuddyStatus(getOption(tmp));
		tmp = "ESC";
		config_options_->setESCommand(getOption(tmp));
	}
}
...
Glib::ustring Config::getOption(Glib::ustring &oname) {
	ifstream config(config_path_.c_str());
	if (!config) {
			cout << "Cannot open config file for reading" << endl;
			perror("Due to");
	}
	string s;
	Glib::ustring tmp;
