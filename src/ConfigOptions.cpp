...
int ConfigOptions::setLogsDirectory(Glib::ustring ldir, int save) {
	...
	if (config_->getApplication()->getBuddyListWin())
		config_->getApplication()->getLog()->setLogPath(ldir_);
...
int ConfigOptions::setEnableEventSounds(Glib::ustring ees, int save) {
	ees_ = ees;
	Glib::ustring tmp("EES");
	if (save)
		return config_->setOption(tmp, ees_, 1);
	else return 1;
}

int ConfigOptions::setESReceivedMessages(Glib::ustring esr, int save) {
	esr_ = esr;
	Glib::ustring tmp("ESR");
	if (save)
		return config_->setOption(tmp, esr_, 1);
	else return 1;
}

int ConfigOptions::setESBuddyStatus(Glib::ustring esb, int save) {
	esb_ = esb;
	Glib::ustring tmp("ESB");
	if (save)
		return config_->setOption(tmp, esb_, 1);
	else return 1;
}

int ConfigOptions::setESCommand(Glib::ustring esc, int save) {
	esc_ = esc;
	Glib::ustring tmp("ESC");
	if (save)
		return config_->setOption(tmp, esc_, 1);
	else return 1;
}
...
int ConfigOptions::setAllToDefaults() {
	...
	setEnableEventSounds("1");
	setESReceivedMessages("1");
	setESBuddyStatus("1");
	setESCommand("/usr/bin/play");
...
void ConfigOptions::syncConfigFromConfigOptions() {
	...
	setEnableEventSounds(getEnableEventSounds());
	setESReceivedMessages(getESReceivedMessages());
	setESBuddyStatus(getESBuddyStatus());
	setESCommand(getESCommand());
}
...
void ConfigOptions::syncConfigOptionsFromConfig() {
	...
	tmp = "EES";
	setEnableEventSounds(config_->getOption(tmp), 0);
	tmp = "ESR";
	setESReceivedMessages(config_->getOption(tmp), 0);
	tmp = "ESB";
	setESBuddyStatus(config_->getOption(tmp), 0);
	tmp = "ESC";
	setESCommand(config_->getOption(tmp), 0);
