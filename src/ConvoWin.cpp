...
ConvoWin::ConvoWin(Glib::RefPtr<Gnome::Glade::Xml> refXml, Application *app, FireClient *client)
	: convowin_(0), windowMinimized(0), convonotebook_(0), markupCount(0), hasFocus(0) {
	app_ptr_ = app;
	client_ = client;
	refXml_ = refXml;
	...
	convowin_->signal_focus_in_event().connect_notify(sigc::mem_fun(*this, &ConvoWin::onWindowFocus));
	convowin_->signal_focus_out_event().connect_notify(sigc::mem_fun(*this, &ConvoWin::onWindowFocus));
...
void ConvoWin::onWindowFocus(GdkEventFocus *event) {
	if (event->in) {
		hasFocus = 1;
		if (!markupCount) {
...
Gtk::Notebook_Helpers::PageIterator ConvoWin::appendPage(Gtk::TreeModel::iterator &iter) {
	...
	page_label->set_text((*iter)[m_Columns.m_col_username]);
	page_label->set_use_markup(0);
